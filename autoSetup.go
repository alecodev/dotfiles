...
func setupENV() bool {
	...
	file := map[string]string{
		...
		"4) Set layout keyboard ":             "sudo localectl set-x11-keymap es",
		"5) Set lightdm greeter-session":      "sudo sed -i 's/#greeter-session=example-gtk-gnome/greeter-session=lightdm-gtk-greeter/' /etc/lightdm/lightdm.conf",
		"6) Set lightdm display-setup-script": "sudo sed -i 's/#display-setup-script=/display-setup-script=/usr/bin/setxkbmap es/' /etc/lightdm/lightdm.conf",
		"7) Set lightdm greeter-setup-script": "sudo sed -i 's/#greeter-setup-script=/greeter-setup-script=/usr/bin/numlockx on/' /etc/lightdm/lightdm.conf",
		"8) Enable lightdm service":           "sudo systemctl enable lightdm",
	}
